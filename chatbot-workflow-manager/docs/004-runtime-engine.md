...
`chatbot.engine.max-steps-per-turn` (50) e `chatbot.engine.max-call-depth` (10) protegem contra
...
`chatbot.engine.max-steps-per-turn` (padrão 50). Nada no schema impede `a → b → a`: sem o teto, esse
