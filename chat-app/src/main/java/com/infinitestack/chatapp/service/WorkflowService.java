...
import com.infinitestack.chatapp.engine.WorkflowParser;
import com.infinitestack.chatapp.engine.WorkflowValidator;
import com.infinitestack.chatapp.repository.SchemaInitializer;
import com.infinitestack.chatapp.repository.WorkflowRepository;
...
public class WorkflowService {
    ...
    public SaveResult save(String pathId, String json) {
        ...
        log.info("[chat-app] workflow gravado | id: {} | nós: {}", workflow.id(), workflow.nodes().size());
    ...
    private List<String> missingSubflowWarnings(Workflow workflow) {
        ...
        for (com.infinitestack.chatapp.domain.Node node : workflow.nodes()) {
            if (node.type() != com.infinitestack.chatapp.domain.NodeType.CALL_WORKFLOW) continue;
    ...
    public boolean delete(String id) {
        ...
        if (removed) log.info("[chat-app] workflow removido | id: {}", id);
        return removed;
    }
    ...
    public void seedDemoIfEmpty() {
        ...
        try {
            ...
            if (result.saved()) {
                log.info("[chat-app] workflow de demonstração inserido | id: {}", result.id());
            } else {
                log.warn("[chat-app] workflow de demonstração inválido — não inserido: {}", result.errors());
            }
        } catch (Exception e) {
            log.warn("[chat-app] falha ao inserir o workflow de demonstração: {}", e.getMessage());
        }
    }
