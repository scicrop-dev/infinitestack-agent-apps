...
Só `SEND_MESSAGE` vira linha de diálogo em `chatapp_event`. `WAIT_INPUT` e `END` são transições de
...
Propriedades: `chatapp.actions.db.{enabled,max-rows,timeout-seconds}` e
`chatapp.actions.http.{enabled,allowed-hosts,timeout-seconds,max-response-bytes}`.
